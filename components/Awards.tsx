  ...
  return (
    <section className="py-24 px-6 md:px-12 lg:px-24 bg-[#F4F4F0] text-[#1A1A1A]">
      <div className="text-center mb-16">
        <span className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-2 block">[ Award Winning ]</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className="space-y-8">
          {awards.map((award, index) => (
            ...
            >
              <div>
                <span className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-1 block">{award.category}</span>
                <h3 className="font-serif text-2xl md:text-3xl group-hover:translate-x-2 transition-transform duration-300">{award.title}</h3>
              </div>
              <span className="text-sm font-bold text-gray-400 group-hover:text-black transition-colors">{award.year}</span>
            </motion.div>
          ))}
        </div>
